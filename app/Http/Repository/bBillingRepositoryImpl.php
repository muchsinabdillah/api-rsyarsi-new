...
class bBillingRepositoryImpl implements bBillingRepositoryInterface
{
    ...
    {
        return  DB::connection('sqlsrv11')->table("FO_T_BILLING_1")->insertGetId([
    ...
    public function updateHeader($request,$notrsbilling)
    {
        return  DB::connection('sqlsrv11')->table("FO_T_BILLING")
        ->where('NO_TRS_BILLING',$notrsbilling)
        ->update([
            //'TGL_BILLING' => $request->TransactionDate,
            //'PETUGAS_ENTRY' =>  $request->UserCreate,
            //'NO_MR' => $request->NoMr,
    ...
    ->where('NO_TRS_BILLING',$NO_TRS_BILLING)
    ...
    public function updateDetailPdp($request)
    {
        return  DB::connection('sqlsrv11')->table("FO_T_BILLING_2")
        ->where('NO_TRS_BILLING',$request->NO_TRS_BILLING)
        ->where('KODE_TARIF',$request->KODE_TARIF)
        ->update([
            'KODE_KOMPONEN_TARIF' => $request->KD_TIPE_PDP,
            'UNIT' => $request->UNIT,  
            'GROUP_JAMINAN' => $request->GROUP_JAMINAN,  
